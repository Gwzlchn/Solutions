...
//are supposed to output the resulting books, sorted in increasing order of their ID's.
...
template<typename T>
void find_map(map<T,set<int>> qmap, T query){
    if(qmap.find(query) == qmap.end()){
        cout<<"Not Found"<<endl;
    }else{
        for(auto it = qmap[query].begin(); it != qmap[query].end(); it++)
            printf("%07d\n", *it);
    }
    return;
}
...
int main(){
    ...
    map<string,set<int> > title_id,author_id,key_id,pub_id,year_id;
    ...
    while(cnt--){
        ...
        stringstream ss(key);
        string key_item;
        while(getline(ss,key_item,' ')){
            key_id[key_item].insert(id);
        }
    ...
    for(int i=1;i<=query_cnt;i++){
        ...
        {
            case 1:
                find_map<string>(title_id,query);break;
            case 2:
                find_map<string>(author_id,query);break;
            case 3:
                find_map<string>(key_id,query);break;
            case 4:
                find_map<string>(pub_id,query);break;
            case 5:{
                
                find_map<string>(year_id,query);break;
            }
